...
def _has_timesheet_view_global(db: Session, requester: Optional["models.EmpDet"]) -> bool:
    ...
    is NOT enough by itself — this privilege must also be present."""
    if not requester:
        return False
    privileges = get_user_privileges(requester, db)
    priv = next((p for p in privileges if p.get("mod_id") == 12), None)
    if not priv:
        return False
    return priv.get("view_prv") in (1, 5)
...
def get_timesheet_employees(
    ...
    Scoping: a global admin sees every employee. Anyone else must have BOTH
    the Timesheet View (Global) Role Privilege AND an actual Assign Manager /
    Project Manager / HR SPOC relationship to an employee — a department-wide
    privilege grant alone must never expose the full employee list to peers
    who don't actually manage anyone.
    """
